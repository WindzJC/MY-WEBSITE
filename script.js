document.addEventListener("DOMContentLoaded", () => {
  ...
  // Featured work carousel (Trailers / Author Websites)
  const workSection = document.getElementById("work");
  const workCarousel = document.querySelector('[data-carousel="featured-work"]');

  if (workSection && workCarousel) {
    const track = workCarousel.querySelector("[data-carousel-track]");
    const slides = Array.from(workCarousel.querySelectorAll("[data-carousel-slide]"));
    const dots = Array.from(workCarousel.querySelectorAll("[data-carousel-dot]"));
    const prevBtn = workSection.querySelector("[data-carousel-prev]");
    const nextBtn = workSection.querySelector("[data-carousel-next]");

    if (track && slides.length) {
      let activeSlide = 0;
      let carouselTicking = false;

      const clampIndex = (idx) => Math.max(0, Math.min(slides.length - 1, idx));

      const updateCarouselUI = (idx) => {
        activeSlide = clampIndex(idx);

        if (prevBtn) prevBtn.disabled = activeSlide === 0;
        if (nextBtn) nextBtn.disabled = activeSlide === slides.length - 1;
      ...
      const syncIndexFromScroll = () => {
        ...
        updateCarouselUI(nearest);
      };

      if (prevBtn) {
        prevBtn.addEventListener("click", () => scrollToSlide(activeSlide - 1));
      }
      if (nextBtn) {
        nextBtn.addEventListener("click", () => scrollToSlide(activeSlide + 1));
      }

      dots.forEach((dot) => {
        dot.addEventListener("click", () => {
          const idx = Number(dot.getAttribute("data-carousel-dot"));
          if (Number.isFinite(idx)) scrollToSlide(idx);
        });
      });
